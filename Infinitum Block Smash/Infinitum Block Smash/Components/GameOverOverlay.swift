...
struct GameOverOverlay: View {
    ...
    // Add new properties for game over summary
    ...
    // Computed property to generate game over summary
    private var gameOverSummary: String {
        if isTimedMode {
            if timeRemaining <= 0 {
                return "Time ran out!"
            } else {
            ...
            return "No more moves available"
        }
    }
    ...
    var body: some View {
        if isPresented {
            GeometryReader { geometry in
                ...
                .zIndex(1000)
                .onAppear {
                    print("[GameOverOverlay] body evaluated with isPresented = \(isPresented)")
...
#Preview {
    GameOverOverlay(
        ...
        isTimedMode: false,
        timeRemaining: 0,
        breakdown: nil // or ScoringBreakdown.example if you have a sample instance
    )
}
#endif
